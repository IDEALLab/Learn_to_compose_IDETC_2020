...
# mesh = Mesh("/home/jun/Documents/Magneto/FEniCSSimulations/AutoWriter/mesh/StrTube.xml")
...
# edge-wise location of inlet of pipes (19 segments at inlet, totally 21 segments on boundary based on the resolution) 
...
class point_BC1(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.22) and near(x[0], lefBound)
class point_BC2(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.23) and near(x[0], lefBound)
class point_BC3(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.24) and near(x[0], lefBound)
class point_BC4(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.25) and near(x[0], lefBound)
class point_BC5(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.26) and near(x[0], lefBound)
class point_BC6(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.27) and near(x[0], lefBound)
class point_BC7(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.28) and near(x[0], lefBound)
class point_BC8(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.29) and near(x[0], lefBound)
class point_BC9(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.30) and near(x[0], lefBound)
class point_BC10(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.31) and near(x[0], lefBound)
class point_BC11(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.32) and near(x[0], lefBound)
class point_BC12(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.33) and near(x[0], lefBound)
class point_BC13(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.34) and near(x[0], lefBound)
class point_BC14(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.35) and near(x[0], lefBound)
class point_BC15(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.36) and near(x[0], lefBound)
class point_BC16(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.37) and near(x[0], lefBound)
class point_BC17(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.38) and near(x[0], lefBound)
class point_BC18(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.39) and near(x[0], lefBound)
class point_BC19(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.40) and near(x[0], lefBound)
class point_BC20(SubDomain):
    def inside(self, x, on_boundary):
        return near(x[1], 0.41) and near(x[0], lefBound)
    
# facetfunc = MeshFunction("size_t", mesh, mesh.geometry().dim()-1)
# bnd = inlet_BC20()
# bnd.mark(facetfunc, 3)

# dtv = dof_to_vertex_map(W)
# w.vector().set_local(subd.array()[dtv])
# File("points.pvd") << w

# File("output/DeCNN_database/DeCNN_simulation_output/inVel_StrTube_test_para/facetfunc.pvd").write(facetfunc)

if __name__ == "__main__":
    #random.seed(0)
    
    outerFolderName = "output/DeCNN_database_journal/DeCNN_simulation_output/StrTube_sepSimu_dis_first/"
...
#        Vel_mult = 1.0
        ...
        for path, _, files in os.walk("output/DeCNN_database_journal/DeCNN_simulation_input/StrTube_inVel_first"):
            for file in glob.glob(os.path.join(path, "*.npy")):
                ...
                # velocity_x = velocity_field[:, 0]
                # velocity_x = dlib.vector(velocity_x)
                ...
                # velocity_y = velocity_field[:, 1]
                # velocity_y = dlib.vector(velocity_y)
                ...
                # inVelVal2 = Expression(('Vel_mult*' + inflowVelExpression(k), '0.0'), Vel_mult = Vel_mult, degree=2) # Inlet velocity
                # inVelVal = Expression(('velocity_x','velocity_y'), velocity_x = velocity_x, velocity_y = velocity_y, degree=2)

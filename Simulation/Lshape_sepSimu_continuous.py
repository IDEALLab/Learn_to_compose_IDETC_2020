...
mesh = Mesh("/home/Magneto/FEniCSSimulations/AutoWriter/mesh/Lshape_new.xml") # Load mesh
...
#    bc = [DirichletBC(W.sub(0), inVelVal, Inlet()),
          DirichletBC(W.sub(1), outPreVal, Lshape_out()),
          DirichletBC(W.sub(0), noSlip.value, notBC(Inlet(), Lshape_out()))]
    solve(F == 0, w, bcs=bc)
...
#    def inside(self, x, on_boundary):
#        return on_boundary and ((coords_inlet[0,0] <= x[1] <= coords_inlet[0,1] and near(x[0], lefBound)))
...
if __name__ == "__main__":
    ...
    outerFolderName = "output/Lshape_sepSimu_new_next/"
...
#    meshFileXML << mesh
    ...
    for z in range(1): # number of iterations
        ...
        i = 1
        rho = interpolate(Expression("x[0] >= 0.0 && x[1] >= 0.21", degree = 1), A) # L-shape up
...
#            inVelVal = Expression(('Vel_mult*' + inflowVelExpression(k), '0.0'), Vel_mult = Vel_mult, degree=2)
            ...
            noSlip = fluidVelBC(value = ('0.0', '0.0')) # Velocity of no slip
...
#            f.write(str(z) + ', ' + str(k) + ', ' + str(Vel_mult) + '\n')
#            f.close()

...
mesh = Mesh("/home/jun/Documents/Magneto/FEniCSSimulations/AutoWriter/mesh/composition_L_L_new.xml") # Load mesh
...
class LshapeD_in(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((0.21 < x[1] < 0.42 and near(x[0], lefBound)))

# Outlet boundary region
class LshapeD_out(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((0.84 < x[1] < 1.05 and near(x[0], lefBound)))
...
if __name__ == "__main__":
    ...
    outerFolderName = "output/GNN_database_dis/GNN_simulation_output/LshapeD_bigSimu_com_dis_div/"
...
#    meshFileXML << mesh
    ...
    outObj = LshapeD_out()
    rho = interpolate(Expression("x[1] > 0.0", degree = 1), A) # L-shape up
    
    for path, _, files in os.walk("output/GNN_database_dis/GNN_simulation_input/Lshape_outVel_npy_next"):
...
#            print(velocity_x1, velocity_x2, velocity_x3, velocity_x4, velocity_x5, velocity_x6, velocity_x7, velocity_x8,
#                  velocity_x9, velocity_x10, velocity_x11, velocity_x12, velocity_x13, velocity_x14, velocity_x15, velocity_x16,
#                  velocity_x17, velocity_x18, velocity_x19, velocity_x20)

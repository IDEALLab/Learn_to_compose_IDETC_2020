...
mesh = Mesh("/home/jun/Documents/Magneto/FEniCSSimulations/AutoWriter/mesh/Lshape_mirror.xml") # Load mesh
...
class Lshape_out(SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and ((0.21 < x[0] < 0.42 and near(x[1], botBound)))
...
if __name__ == "__main__":
    ...
    outerFolderName = "output/DeCNN_database_journal/DeCNN_simulation_output/LshapeM_sepSimu_dis_first/"
...
#        i = 1
        rho = interpolate(Expression("x[1] < 0.63", degree = 1), A) # L-shape up
        
#        Vel_mult = random.uniform(0.0, 1.0) #0.01 # Change here
#        Vel_mult = 1.0  
        
        for path, _, files in os.walk("output/DeCNN_database_journal/DeCNN_simulation_input/LshapeM_inVel_first"):
